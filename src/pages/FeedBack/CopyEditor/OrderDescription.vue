...
export default {
  ...
  methods: {
    ...
    uploadImg(formdata) {
      ...
      copyEditor.orderDescriptionUploadImage(params).then(res => {
        if (res.code === 200) {
          // 图片的远程路径
          let value = res.data;
          // // 将图片添加到富文本内容区域
    ...
    getCopywriting() {
      ...
      copyEditor.getOneOrderDescriptionById({ id: this.copywritingId }).then(res => {
    ...
    handleSave() {
      this.$refs["form"].validate(async valid => {
        if (valid) {
          ...
          copyEditor.saveOrUpdateOrderDescription(params).then(res => {
            if (res.code == 200) {
              this.$message.success(this.copywritingId?"编辑成功":'新增成功');
              this.handleCancel();
            }else{
              this.$message.error(res.msg)
            }
