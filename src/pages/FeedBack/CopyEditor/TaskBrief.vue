...
export default {
  ...
  methods: {
    ...
    uploadImg(formdata) {
      ...
      copyEditor.taskDescriptionUploadImage(params).then(res => {
        if (res.code === 200) {
          // 图片的远程路径
          let value = res.data;
          // // 将图片添加到富文本内容区域
    ...
    getCopywriting() {
      ...
      copyEditor.getOneTaskDescriptionById({ id: this.copywritingId }).then(res => {
    ...
    handleSave() {
      this.$refs["form"].validate(async valid => {
        if (valid) {
          ...
          copyEditor.saveOrUpdateTaskDescription(params).then(res => {
            if (res.code == 200) {
