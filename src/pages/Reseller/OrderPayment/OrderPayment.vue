  ...
  <el-container>
    <el-header class="header">
      <el-row type="flex" class="row-bg" justify="space-between">
        ...
        <el-col :span="7" :offset="5" class="text-right">
          <el-input v-model="search" placeholder="请输入要查询的京店宝手机号" size="small" clearable></el-input>
    ...
    <el-main>
      <el-table :data="tableData">
        <el-table-column prop="phone" label="京店宝手机号"></el-table-column>
        <el-table-column label="待付款时间" prop="paymentTime"></el-table-column>
        ...
        <el-table-column label="操作时间" prop="changeTime"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    followOrder(id) {
      reseller
        .getDocumentary({ id, userId: this.jdb_userInfo.id })
        .then(res => {
          if (res.code === 200) {
            this.$message({
              type: "success",
    ...
    resultFn(status, id) {
      reseller
        .result({ id, status, userId: this.jdb_userInfo.id })
        .then(res => {
          if (res.code === 200) {
            this.$message({
